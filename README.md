# ImagePicker
自定义相册，完全仿微信UI，实现了拍照、图片选择（单选/多选）、 裁剪 、旋转、等功能。
...
## 演示
 ![image](image/demo1.png)![image](image/demo2.gif)
 ![image](image/demo3.gif)![image](image/demo5.gif)

## 1.用法

使用前，对于DevEco Studio的用户，可以选择添加:
```java
...
2. 然后配置图片选择器，一般在`AbilityPackage`初始化配置一次就可以,这里就需要将上面的图片加载器设置进来,其余的配置根据需要设置
```java
...
V 0.6.1
 * [合并] 优化图片选择页UI， 适配预览页的横竖屏切换

V 0.6.0
 * [合并] 调整UI,真正的完全仿微信
 * [合并] fix(location): 解决不合法图片导致的Bug

...
/**
 * Presents load'n'display image task. Used to load image from Internet or file system, decode it to {@link PixelMap}, and
...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
    ...
    /**
     * Load and display image task
     *
     * @param engine           engine
     * @param imageLoadingInfo image loading info
     * @param handler          handler
     */
    public LoadAndDisplayImageTask(ImageLoaderEngine engine, ImageLoadingInfo imageLoadingInfo, EventHandler handler) {
        this.engine = engine;
    ...
    private PixelMap tryLoadBitmap() throws TaskCancelledException {
        ...
        try {
            ...
            if (bitmap == null || bitmap.getImageInfo().size.width <= 0 || bitmap.getImageInfo().size.height <= 0) {
    ...
    private PixelMap decodeImage(String imageUri) throws IOException {
    ...
    private boolean resizeAndSaveImage(int maxWidth, int maxHeight) throws IOException {
        ...
        if (targetFile != null && targetFile.exists()) {
            ...
            PixelMap bmp = decoder.decode(decodingInfo);
     ...
     * @param engine  engine
     */
    static void runTask(Runnable r, boolean sync, EventHandler handler, ImageLoaderEngine engine) {
        if (sync) {
            r.run();
